...
Implement the changes described above. Then git add and git commit the changes."
  ...
  if command -v gh &>/dev/null; then
    ...
    if [[ "$PR_URL_OUTPUT" == https://* ]]; then
      PR_HTML_URL="$PR_URL_OUTPUT"
      PR_NUMBER=$(echo "$PR_URL_OUTPUT" | grep -oE '[0-9]+$')
    fi
  fi

  if [[ -z "$PR_NUMBER" && -n "$TOKEN" ]]; then
  ...
  if [[ -n "$PR_NUMBER" ]]; then
    ...
    if command -v gh &>/dev/null; then
      gh pr review "$PR_NUMBER" --repo "$REPO" --request --reviewer mindthevirt 2>/dev/null || true
    fi
  else
    fail "Failed to create PR (check token scopes — needs 'repo' scope)"
    git checkout "$BASE_BRANCH" 2>/dev/null || true
    git branch -D "$BRANCH" 2>/dev/null || true
    continue
  fi
